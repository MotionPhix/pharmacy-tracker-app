<script setup lang="ts">
import { inject, ref } from 'vue'

interface PharmacyInterface {
  pharmacies?: {
    id: Number
    name: String
    address: String
    lat?: String
    lon?: String
  }[]
}

const props = withDefaults(defineProps<PharmacyInterface>(), {
  pharmacies: () => [],
})

const Splade = inject('$splade')

const selectedPharmacy = ref(props.pharmacies[0])

const findPharmacy = (pharmacy) => {
  Splade.modal(`/pharmacies/${pharmacy.id}`)
}
</script>

<template>
  <Listbox v-model="selectedPharmacy">
    <div class="relative mt-1">
      <ListboxButton
        class="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm"
      >
        <span class="block truncate">{{ selectedPharmacy.name }}</span>
        <span
          class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2"
        >
          <ChevronUpDownIcon
            class="h-5 w-5 text-gray-400"
            aria-hidden="true"
          />
        </span>
      </ListboxButton>

      <transition
        leave-active-class="transition duration-100 ease-in"
        leave-from-class="opacity-100"
        leave-to-class="opacity-0"
      >
        <ListboxOptions
          class="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
        >
          <ListboxOption
            v-for="pharmacy in pharmacies"
            v-slot="{ active, selected }"
            :key="pharmacy.name"
            :value="pharmacy"
            as="template"
          >
            <li
              class="relative cursor-default select-none py-2 pl-10 pr-4" :class="[
                active ? 'bg-amber-100 text-amber-900' : 'text-gray-900',
              ]"
              @click="findPharmacy(pharmacy)"
            >
              <span
                class="block truncate" :class="[
                  selected ? 'font-medium' : 'font-normal',
                ]"
              >{{ pharmacy.name }}</span>
              <span
                v-if="selected"
                class="absolute inset-y-0 left-0 flex items-center pl-3 text-amber-600"
              >
                <CheckIcon class="h-5 w-5" aria-hidden="true" />
              </span>
            </li>
          </ListboxOption>
        </ListboxOptions>
      </transition>
    </div>
  </Listbox>
</template>
